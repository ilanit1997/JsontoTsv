...
def main():
    ...
    with open(path, encoding='utf8') as jsonfile:
        ...
        ordered_headeres = [h.split('.')[0] for h in headers]
        with open('out.tsv', 'w', encoding='utf8') as tsvfile:
            writer = csv.writer(tsvfile, delimiter='\t')
            writer.writerow(h for h in headers)
            for d in indata:
                d = sort_dict(d, ordered_headeres) #sort keys to keep order in dict
...
def sort_dict(dict, order):
    """
    This function sorts dict by new order of keys
    :param dict: dict to sort key by
    :param order: ordered list with headers
    :return: new ordered dict by keys in order
    """
    newd = OD()
    for i in order:
        newd[i] = dict[i]
